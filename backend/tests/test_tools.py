...
def test_move_invalid_exit_rejected(db: Session, game: Game):
    ...
    assert game.location == "cave_entrance"  # unchanged
...
def test_pickup_only_present_items(db: Session, game: Game):
    # No iron_sword at the cave entrance.
...
def test_attack_requires_enemy_present(db: Session, game: Game):
    ...
    assert res["ok"] is False  # no goblin at the cave entrance
...
def test_apply_damage_only_player_and_bounded(db: Session, game: Game):
    assert _exec(db, game, "apply_damage", target="goblin", amount=10)["ok"] is False
    assert _exec(db, game, "apply_damage", target="player", amount=9999)["ok"] is False
    assert _exec(db, game, "apply_damage", target="player", amount=-5)["ok"] is False
...
def test_full_win_path(db: Session, game: Game):
    # cave_entrance -> hall -> armory (sword) -> hall -> goblin_lair (kill) -> hall -> treasure_vault -> claim relic
    _exec(db, game, "move_player", direction="north")
    _exec(db, game, "move_player", direction="east")
    assert _exec(db, game, "pick_up_item", item="iron_sword")["ok"] is True
    ...
    res = _exec(db, game, "move_player", direction="north")  # treasure_vault, now unlocked
    assert res["ok"] is True
    assert game.location == "treasure_vault"
    assert game.status == GameStatus.active
    claim = _exec(db, game, "pick_up_item", item="ancient_relic")
